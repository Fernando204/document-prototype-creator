...
const Saude = () => {
  ...
  const filteredEvents = events
    .filter((event) => {
      const horseNames = getHorseNames(event.horseIds);
      const matchesSearch =
        event.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
        horseNames.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <Input placeholder="Buscar por título ou cavalo..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
          </div>
        ...
        <div className="space-y-3">
          {filteredEvents.length === 0 ? (
            ...
            filteredEvents.map((event) => {
              ...
              return (
                <div key={event.id} className="bg-card rounded-xl shadow-soft p-4 flex items-center gap-4 hover:shadow-card transition-shadow">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium">{format(new Date(event.date), "dd MMM", { locale: ptBR })}</p>
                    {event.time && <p className="text-xs text-muted-foreground">{event.time}</p>}
                    {event.cost && <p className="text-xs text-horse-sage font-medium">R$ {event.cost.toFixed(2)}</p>}
                  </div>
                  <div className="flex gap-2">
                    {event.status === "agendado" && (
                      <Button size="sm" variant="outline" onClick={() => completeEvent(event.id)}>
                        <CheckCircle2 className="h-4 w-4" />
                      </Button>
                    )}
                    <Button size="sm" variant="ghost" className="text-destructive hover:text-destructive" onClick={() => deleteEvent(event.id)}>
