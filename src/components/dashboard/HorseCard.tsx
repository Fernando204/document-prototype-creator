    ...
    <div className="group bg-card rounded-xl shadow-soft hover:shadow-card transition-all duration-300 overflow-hidden animate-fade-in">
      ...
      <div className="relative h-40 bg-muted overflow-hidden">
        {imageUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-horse-cream to-muted">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-16 w-16 text-horse-brown-light/30">
        ...
        <Badge className={cn("absolute top-3 left-3 gap-1.5 text-xs font-medium", config.color)}>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" className="absolute top-2 right-2 h-8 w-8 bg-background/50 backdrop-blur-sm hover:bg-background/80">
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex gap-2 pt-2">
          <Button variant="outline" size="sm" className="flex-1 text-xs h-8" onClick={handleViewDetails}>
            Ver Detalhes
          </Button>
          {onEdit && (
